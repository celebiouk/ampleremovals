  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-brand-purple-50 to-white p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-xl">
        ...
        <p className="mt-3 text-slate-600">Tap the star with your rating — <strong>5 is the best!</strong></p>
        <div className="mt-8 flex justify-center gap-2">
          {[1, 2, 3, 4, 5].map((n) => {
            ...
            return (
              ...
              >
                <Star className={`h-12 w-12 ${active ? "fill-yellow-400 text-yellow-400" : "fill-slate-100 text-slate-300"}`} />
                {/* The rating number sits inside the star (clear on touch, where there's no hover). */}
                <span className={`pointer-events-none absolute inset-0 flex items-center justify-center text-lg font-extrabold ${active ? "text-white" : "text-slate-500"}`}>
                  {n}
                </span>
              </button>
            );
