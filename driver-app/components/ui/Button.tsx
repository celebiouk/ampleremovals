...
import { colors, radius, shadows, spacing, type, MIN_TOUCH } from "@/lib/theme";
...
const VARIANT: Record<Variant, { bg: string; fg: string; border?: string; glow?: object }> = {
  ...
  accent: { bg: colors.accent.DEFAULT, fg: colors.white },
...
const SIZE: Record<Size, { height: number; px: number; radius: number; font: number }> = {
  sm: { height: 42, px: spacing.base, radius: radius.md, font: 14 },
  md: { height: 52, px: spacing.xl, radius: radius.lg, font: 16 },
  lg: { height: 60, px: spacing.xl, radius: radius.lg, font: 18 },
};

export function Button({
  label, variant = "primary", size = "md", loading = false, disabled, icon, onPress, fullWidth, ...props
}: ButtonProps) {
  ...
  return (
    <AnimatedPressable
      ...
      style={[
        animStyle,
        {
          minHeight: Math.max(s.height, MIN_TOUCH), height: s.height, paddingHorizontal: s.px,
          borderRadius: s.radius, backgroundColor: v.bg, borderWidth: v.border ? 2 : 0,
          borderColor: v.border, flexDirection: "row", alignItems: "center", justifyContent: "center",
          gap: spacing.sm, opacity: isDisabled ? 0.5 : 1, width: fullWidth ? "100%" : undefined,
        },
